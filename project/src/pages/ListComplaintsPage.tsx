...
import { Search, Filter, MapPin, ThumbsUp, ChevronLeft, ChevronRight } from 'lucide-react';

type Category = 'All' | 'Roads' | 'Sanitation' | 'Utilities' | 'Public Safety';
...
const ListComplaintsPage = () => {
  ...
  const displayedComplaints = filteredComplaints.slice(startIndex, startIndex + itemsPerPage);

  const getStatusColor = (status: Exclude<Status, 'All'>) => {
    switch (status) {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {displayedComplaints.map((complaint) => (
          <div key={complaint.id} className="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  <span className={`px-2 py-1 rounded-full text-sm font-medium ${getStatusColor(complaint.status)}`}>
                    {complaint.status}
                  </span>
