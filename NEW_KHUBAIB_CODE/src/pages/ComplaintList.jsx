...
const ListComplaintsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let data = await api.getComplaints();
        ...
        if (data.length === 0) {
          for (let complaint of dummyData) {
            await api.createComplaint(complaint);
          }
          const newData = await api.getComplaints();
          setComplaints(Array.isArray(newData) ? newData : []);
        } else {
          setComplaints(data);
        }
      } catch (error) {
